...
/*   Updated: 2022/06/05 13:15:35 by mtiesha          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../include/minishell.h"

static int	check_errno(t_src *src, char *str)
{
...
static void	check_type(t_src *src, char *str, char *path)
{
	DIR			*dir;
	int			fd;
	char		**cmds;
...
static void	set_filename(int len, char **new, char *str)
{
	...
	*new = ft_strjoin(aux, "/");
	...
	free(*new);
	*new = aux;
	free(filename);// ../../../usr/bin/filedir/ls     usr/bin/filedir/ls
}
...
static void	set_path(char *str, char **path)
{
	...
	len = ft_strlen(*path);
	if (!ft_memcmp(str, "/", 1))
		*path = ft_strdup(str);// if str == /bin/ls
	else
	{
		set_filename(len, &new, str);// ../bin/ls || ls
		*path = new;
		return ;
	}
	free(new);
